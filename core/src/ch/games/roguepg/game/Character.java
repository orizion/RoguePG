...
public class Character extends Actor {
    ...
    public Character() {
        ...
        while (RogueMap.tileIndices[(int) bDef.position.x][(int) bDef.position.y] == 0) {
            bDef.position.set(rnd.nextFloat() * (RogueMap.tileIndices.length), rnd.nextFloat()
                    * (RogueMap.tileIndices[0].length));
        }
    ...
    public void draw(Batch batch, float parentAlpha) {
        batch.draw(sprite, (body.getPosition().x - 0.5f) * RoguePG.PPM, (body.getPosition().y - 0.5f) * RoguePG.PPM);
    }
